<<<<<<< HEAD
/* Admin Enhanced Logic - Pro Version */
(function () {
    'use strict';
    ...
    async function loadOverviewData() {
        try {
            ...
            if (result.success && result.data) {
                ...
                document.getElementById('totalCoins').textContent = data.totalCoins || 0;
    ...
    function renderDeviceBreakdown(devices) {
        ...
        const browserIcons = { Chrome: '🌐', Safari: '🧭', Firefox: '🦊', Edge: '🔷', Unknown: '❓' };
    ...
    async function loadUserNotes(userId) {
        const result = await window.SupabaseConfig.getUserNotes(userId);
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach(note => {
                ...
                noteItem.innerHTML = `
                    ...
                    <div class="note-meta">
                        Admin - ${new Date(note.created_at).toLocaleString('vi-VN')}
                    </div>
                `;
    ...
    document.getElementById('btnAddNote').addEventListener('click', async function () {
        ...
        const result = await window.SupabaseConfig.addUserNote(currentUserId, note);
    ...
    document.getElementById('btnResetPassword').addEventListener('click', async function () {
        const authResult = await window.SupabaseConfig.client().auth.admin.getUserById(currentUserId);
        const email = authResult.data.user.email;

        if (!confirm(`Gửi email đặt lại mật khẩu cho ${email}?`)) return;

        const result = await window.SupabaseConfig.adminResetPassword(email);
    ...
    if (btnCalculateStats) {
        btnCalculateStats.addEventListener('click', async function () {
            this.disabled = true;
            this.textContent = '⏳ Đang tính toán...';

            try {
                await window.SupabaseConfig.calculateTodayStats();
                alert('✅ Đã tính toán xong!');
                await loadStatsData();
            } catch (error) {
            ...
            this.disabled = false;
            this.textContent = '🔄 Tính toán lại';
        });
    }
...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', function (e) {
        ...
        // Update page title
        const titles = {
            overview: { title: 'Tổng quan', subtitle: 'Chào mừng trở lại, Admin!' },
            users: { title: 'Người dùng', subtitle: 'Quản lý tất cả người dùng' },
            devices: { title: 'Thiết bị', subtitle: 'Theo dõi thiết bị truy cập' },
            activity: { title: 'Hoạt động', subtitle: 'Xem hoạt động realtime' },
            stats: { title: 'Thống kê', subtitle: 'Phân tích dữ liệu' },
